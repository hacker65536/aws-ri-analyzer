...
| cur_instances | 稼働中インスタンス一覧（実績使用時間・コスト・平均台数） | Athena / CUR |
...
├── .gitignore
├── main.py                            # CLI エントリポイント（CE + Athena 統合）
...
main.py
  ...
  └─ [for each service in config]
      ...
      └─ [Athena/CUR セクション（--athena 時）]
          ├─ running_rds_instances()     # CUR: 稼働中 RDS インスタンス
          │    → parse_rds_instances()   # CurInstanceRow[]
          ├─ ri_coverage_detail()        # CUR: RI カバレッジ詳細
          │    → parse_cur_coverage()    # CurCoverageRow[]
          ├─ unused_ri_cost()            # CUR: 未使用 RI 費用
          │    → parse_unused_ri()       # UnusedRiRow[]
          ├─ factcheck_recommendations() # CE 推奨 × CUR 実績 突き合わせ
...
```sql
WHERE year = '2026' AND month = '3'   -- OK
WHERE year = '2026' AND month = '03'  -- NG（パーティションにヒットしない）
```
...
| サービス | キー |
|---|---|
| elasticache | `ElastiCacheInstanceDetails` → `NodeType`, `Region`, `ProductDescription` |
| rds | `RDSInstanceDetails` → `InstanceType`, `Region`, `DatabaseEngine`, `DeploymentOption` |
...
- `print_cur_instances(rows, service, year, month)`
  - 稼働中インスタンスを engine 別にグループ表示
  - 列: Account ID / Region / Instance Type / Engine / Deployment / Avg Inst / Usage hrs / Cost
  - `avg_instances = usage_hours / 720`（月間平均台数）
- `print_cur_coverage(rows, service, year, month)`
...
- `print_unused_ri(rows, service, year, month)`
  - RIFee 行の費用を降順表示。無駄な RI の特定に使う
  - 列: Account ID / Region / Usage Type / Fee (USD) / Qty
  - フッターに合計未使用 RI 費用
- `print_ce_factcheck(checks, service, year, month)`
  ...
  - Judge の基準:
    ...
    - `[?] no CUR data` : CUR にデータなし（year/month や engine を確認）
...
| 構造体 | 内容 |
|---|---|
| `CurInstanceRow` | 稼働中インスタンス 1 行（account × region × instance_type × engine） |
...
- `athena:instances:{svc}:{year}:{month}` → `list[CurInstanceRow]`
- `athena:coverage:{svc}:{year}:{month}` → `list[CurCoverageRow]`
- `athena:unused_ri:{svc}:{year}:{month}` → `list[UnusedRiRow]`
