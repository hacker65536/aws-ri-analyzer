...
| avg_od_rate | `Attributes["averageOnDemandHourlyRate"]` ← Multi-AZ 判定に使用 |
...
### `ri_analyzer/pricing.py`

AWS Pricing API からオンデマンド時間単価を取得・キャッシュするモジュール。

#### `AwsPricingClient`

| メソッド | 引数 | 説明 |
|---|---|---|
| `get_od_price(service, instance_type, region, engine, deployment)` | service: "rds"/"elasticache"/"opensearch" | オンデマンド時間単価（USD/hr）を返す。取得失敗時は None |

- Pricing API は `us-east-1` エンドポイントのみ
- 料金はほぼ変わらないためデフォルト TTL は 168 時間（7 日）
- `ServiceCode`: `"AmazonRDS"` / `"AmazonElastiCache"` / `"AmazonES"`
- 複数ヒット時（IO 最適化など）は最小値 = 標準価格を採用

#### `annotate_multi_az(subscriptions, pricing_client, service, tolerance)`

RiSubscription の `multi_az` フィールドを in-place で設定する。

**検出方法**: CE `GetReservationUtilization` の `averageOnDemandHourlyRate` 属性（CE が内部的に使用する現在のオンデマンド単価）を、AWS Pricing API から取得した Single-AZ / Multi-AZ のオンデマンド単価と比較することで Multi-AZ かどうかを判定する。tolerance（デフォルト 5%）の範囲内で一致した方を採用する。

**なぜ `averageOnDemandHourlyRate` を使うか**: CE `GetReservationUtilization` の `platform` 属性は `"MySQL"` のようなエンジン名のみで Multi-AZ 情報を含まない。`DEPLOYMENT_OPTION` の GroupBy は API で拒否される（`Only SUBSCRIPTION_ID is supported`）。`averageOnDemandHourlyRate` は CE が節約額計算に使う現在の公開オンデマンド単価であり、価格変動後も常に現在値が返されるため Pricing API との比較が信頼性高く機能する。

| パラメータ | 説明 |
|---|---|
| `subscriptions` | `list[RiSubscription]` — in-place で `multi_az` を更新 |
| `pricing_client` | `AwsPricingClient` インスタンス |
| `service` | `"rds"` のみ有効（他はスキップ） |
| `tolerance` | 価格一致の許容誤差（デフォルト 5%） |

**制約**:
- Aurora の RI は常に Single-AZ のためスキップ（`multi_az = None` のまま）
- `avg_od_rate` が 0 の場合は判定不能（`multi_az = None` のまま）
- `multi_az` はキャッシュに保存しない（`avg_od_rate` はキャッシュに含む）。毎回 `annotate_multi_az()` で再計算するが、Pricing API レスポンスは別途 7 日 TTL でキャッシュされる

**必要な IAM 権限**: `pricing:GetProducts`（`us-east-1` エンドポイント）
...
**エンジン・ファミリーフィルタの適用タイミング**: `analyze()` はフィルタを持たない。`--engine` / `--family` によるフィルタは `main.py` が `analyze()` を呼ぶ前に `subscriptions` リストを絞り込むことで実現する。Platform 名は CE の `GetReservationUtilization` が短縮形（例: `"Aurora"`）を返すため、`_normalize_util_platform()` で `"Aurora MySQL"` に正規化してからサブストリング一致を行う。
...
**RDS（Single-AZ）/ ElastiCache Redis OSS（`_NORM_FACTOR`）:**
```
nano=0.25 / micro=0.5 / small=1 / medium=2 / large=4 / xlarge=8 / 2xlarge=16 / 4xlarge=32 / 8xlarge=64 / ...
```

**RDS Multi-AZ DB instance**: Single-AZ の **2 倍**（primary + standby の 2 台分）。
**RDS Multi-AZ DB cluster**: Single-AZ の **3 倍**（primary + standby × 2 の 3 台分）。
参照: [AWS RDS Reserved Instances User Guide](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_WorkingWithReservedDBInstances.html)
...
- `UtilizationSummary.normalized_units` = `count × _norm_factor_for_engine()`（Single-AZ 換算のみ）
- family サマリの `Avg Util` は NUs 加重平均で算出
- **制約**: Utilization セクションの NUs は `RiUtilizationRecord` ベースのため `multi_az` 情報を持たず、Multi-AZ RI でも Single-AZ 換算の NUs を表示する。Expiration セクションは `RiSubscription.multi_az` を参照して正確な NUs（× 2）を表示する
...
#### タイムゾーンユーティリティ（`reporter/_base.py`）

- `set_display_timezone(tz_name: str | None)` — モジュールレベル変数 `_display_tz` を設定する。`tz_name` が `None` の場合は OS ローカル TZ を使用（デフォルト）。`main.py` が `config.analysis.display_timezone` を渡すことで全セクションに反映される
- `to_display_tz(dt: datetime) -> datetime` — UTC-aware な `datetime` を表示用 TZ に変換して返す。`_display_tz` が設定されていない場合は `dt.astimezone()`（OS ローカル TZ）を使用

設定値は `config.yaml` の `analysis.display_timezone` から読み込まれる（IANA タイムゾーン名、例: `"Asia/Tokyo"`）。省略時は OS ローカル TZ が使われるため、設定なしでも環境に応じた正しいTZで日付が表示される。

#### CE セクション（既存）

- `print_expiration(expired, warning, ok, warn_days, engines=None, families=None)`
  - 有効期限切れ / 警告 / 正常の 3 グループを表示
  - `engines` / `families` を指定するとタイトルにフィルタ条件を付記する（絞り込み自体は `main.py` 側で実施済み）
  - 各行の列: Instance Type / Count / NUs / Engine / Region / expires 日付 / 残日数
    - **NUs**: `count × _norm_factor_for_engine(instance_type, platform)` で算出（エンジン別係数適用）
    - **expires**: `to_display_tz(ri.end_time).strftime('%Y-%m-%d')` で表示 TZ に変換済み
  - 残日数表示: 期限切れは `(N days ago)`、未期限は `(N days left)`
...
`us-east-1` エンドポイントで呼び出す。Multi-AZ 判定（`annotate_multi_az`）で使用。
...
| Multi-AZ 情報（Expiration） | `averageOnDemandHourlyRate` vs Pricing API で判定。Aurora はスキップ（常に Single-AZ）。`avg_od_rate=0` や価格取得失敗時は `None`（不明）のまま表示 |
| Multi-AZ 情報（Coverage） | CE の `GetReservationCoverage` では Multi-AZ フラグが取得できないため、カバレッジ分析では考慮されていない |
