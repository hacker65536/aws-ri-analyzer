...
> 以降のコマンドは `uv run python ...` で実行するか、先に `source .venv/bin/activate` で venv を有効化してください。
...
# セクションを絞って実行
uv run python main.py --section expiration utilization

# 利用率が低い RI だけ表示
uv run python main.py --section utilization --max-util 80

# カバレッジが低いグループだけ表示
uv run python main.py --section coverage --max-coverage 90

# カバレッジを特定エンジン・ファミリーに絞って表示
uv run python main.py --section coverage --engine aurora
uv run python main.py --section coverage --engine "aurora mysql" --family r6g r8g

# キャッシュを無視して AWS から再取得
uv run python main.py --no-cache

# カラーなし出力（ログ保存・CI 用）
uv run python main.py --no-color > output.log
```
...
uv run python main.py --service rds --athena

# 年月を明示指定
uv run python main.py --service rds --athena --cur-year 2026 --cur-month 3

# CUR セクションだけ表示
uv run python main.py --service rds --section cur_instances cur_coverage unused_ri

# CE Recommendation + CUR ファクトチェック
uv run python main.py --service rds --section recommendations --athena
```
