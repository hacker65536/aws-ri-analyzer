-- テンプレート: ElastiCache ノード リソース ID 一覧（OD / RI 内訳付き）
...
--
-- 例:
--   python athena_run.py elasticache_resource_ids \
--     -p instance_type_prefix=cache.m7g \
--     -p engine=redis
...
WHERE year = '{{ year }}'
  ...
  AND line_item_usage_type LIKE '%NodeUsage%'
  AND product_instance_type LIKE '{{ instance_type_prefix }}.%'
  AND LOWER(product_cache_engine) LIKE '%{{ engine }}%'
